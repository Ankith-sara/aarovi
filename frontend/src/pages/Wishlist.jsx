...
const Wishlist = () => {
  ...
  const handleAddToCart = async () => {
    ...
    if (ok) { toast.success('Added to cart!'); closeSizeModal(); }
  };
  ...
  return (
    <div className="min-h-screen bg-primary pt-20 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 mt-4">
          <div>
            ...
            <p className="text-sm text-text/40 mt-1">{wishlistProducts.length} {wishlistProducts.length === 1 ? 'item' : 'items'}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex rounded-full border border-stone-200 overflow-hidden">
              <button onClick={() => setViewMode('grid')} className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-secondary text-background' : 'bg-white text-stone-500'}`}><Grid3x3 size={15} /></button>
              <button onClick={() => setViewMode('list')} className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-secondary text-background' : 'bg-white text-stone-500'}`}><List size={15} /></button>
            </div>
          </div>
        </div>
        ...
        <div className={`grid gap-4 sm:gap-6 ${viewMode === 'grid' ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1 max-w-2xl'}`}>
          {wishlistProducts.map(product => (
            <div key={product._id} className={`bg-white border border-stone-100 rounded-2xl overflow-hidden hover:shadow-md transition-all group ${viewMode === 'list' ? 'flex gap-4 p-4' : ''}`}>
              {/* Image */}
              <Link to={`/product/${product._id}`} className={`block relative overflow-hidden bg-stone-50 ${viewMode === 'list' ? 'w-28 h-28 flex-shrink-0 rounded-xl' : 'aspect-[3/4]'}`}>
                <img
              ...
              <div className={`${viewMode === 'list' ? 'flex-1 flex flex-col justify-between py-1' : 'p-3 sm:p-4'}`}>
                <div>
                  <Link to={`/product/${product._id}`} className="block">
                    <h3 className="text-sm font-medium text-text leading-snug line-clamp-2 hover:text-secondary transition-colors">{product.name}</h3>
                  </Link>
                  <p className="text-base font-semibold text-secondary mt-1.5">{currency}{product.price?.toLocaleString()}</p>
                </div>
                <div className={`${viewMode === 'list' ? 'flex items-center gap-2 mt-3' : 'mt-3 space-y-2'}`}>
                  ...
                  {viewMode === 'list' && (
                    <button onClick={() => removeFromWishlist(product._id)} className="p-2.5 border border-stone-200 rounded-xl hover:bg-red-50 hover:border-red-200 transition-colors">
      ...
      {showSizeModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={closeSizeModal}>
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-sm p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-text">Select Size</h3>
              <button onClick={closeSizeModal} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-stone-100"><X size={16} /></button>
            </div>
            <div className="flex gap-2 flex-wrap mb-6">
              {selectedProduct.sizes?.map(s => (
                <button
                  key={s} onClick={() => setSelectedSize(s)}
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium border-2 transition-all ${selectedSize === s ? 'border-secondary bg-secondary text-background' : 'border-stone-200 text-text hover:border-secondary/50'}`}
                >
                  {s}
                </button>
              ))}
            ...
            <button
              ...
              className="w-full py-3.5 bg-secondary text-background rounded-xl font-semibold text-sm hover:bg-secondary/90 transition-all flex items-center justify-center gap-2"
            >
