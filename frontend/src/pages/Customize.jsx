...
import { Save, ShoppingCart, Upload, CheckCircle2, Ruler, Palette, Shirt, ArrowRight, Sparkles, IndianRupee } from "lucide-react";
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setForm({ ...form, designNotes: data.designSuggestions || form.aiPrompt });
      ...
      setForm({ ...form, designNotes: form.aiPrompt });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              ...
              return (
                <React.Fragment key={s.num}>
                  <div className="flex flex-col items-center">
                    <div className={`relative w-16 h-16 rounded-full flex items-center justify-center font-bold text-lg transition-all duration-300 ${
                        step === s.num ? "bg-secondary text-white shadow-xl shadow-secondary/30 scale-110" :
                        step > s.num ? "bg-green-500 text-white shadow-lg" : "bg-background/30 text-text/40"
                      }`}>
                      {step > s.num ? <CheckCircle2 size={28} /> : <Icon size={28} />}
                    </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-background/50 overflow-hidden">
          {step === 1 && (
            <div className="p-8 sm:p-12 space-y-10">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-serif font-bold text-text mb-2">Tell Us Your Preferences</h2>
                <p className="text-text/60 font-light">Select your style essentials to get started</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {["Women", "Men"].map((gender) => (
                    <button key={gender} onClick={() => handleGenderChange(gender)}
                      ...
                      {form.gender === gender && (
                        <div className="absolute top-4 right-4"><CheckCircle2 size={24} className="text-secondary" /></div>
                      )}
                    </button>
                  ))}
                </div>
                    ...
                    {dressTypes[form.gender].map((dress) => (
                      <button key={dress.value} onClick={() => handleDressTypeChange(dress.value)}
                        ...
                        {form.dressType === dress.value && (
                          <div className="absolute top-3 right-3"><CheckCircle2 size={20} className="text-secondary" /></div>
                        )}
                      </button>
                    ))}
                  </div>
                      ...
                      return (
                        <button key={fabric.value} onClick={() => setForm({ ...form, fabric: fabric.value })}
                          ...
                          {form.fabric === fabric.value && (
                            <div className="absolute top-4 right-4"><CheckCircle2 size={20} className="text-secondary" /></div>
                          )}
                        </button>
                      );
                    })}
          ...
          {step === 2 && (
            <div className="p-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-serif font-bold text-text mb-2">Customize Your Design</h2>
                <p className="text-text/60 font-light">Select your color and customize patterns on different zones</p>
              </div>
          ...
          {step === 3 && (
            <div className="p-8 sm:p-12 space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-serif font-bold text-text mb-2">Final Touches</h2>
                <p className="text-text/60 font-light">Add notes, measurements, and reference images for perfect fit</p>
              </div>
              ...
              {estimatedPrice > 0 && (
                <div className="bg-gradient-to-br from-secondary/5 to-white rounded-2xl p-6 border-2 border-secondary/20 shadow-lg">
                  <h3 className="flex items-center gap-2 text-xl font-serif font-semibold mb-4 text-text">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-text/60 font-light mb-1">Based on your selections:</p>
                      <p className="text-text/70 text-sm">{form.dressType} • {form.fabric}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-4xl font-bold text-secondary">₹{estimatedPrice.toLocaleString()}</p>
                      <p className="text-xs text-text/50 mt-1 font-light">Final price may vary</p>
                    </div>
                  </div>
                </div>
              )}
